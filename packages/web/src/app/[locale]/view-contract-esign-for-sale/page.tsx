...
export default function ViewContractESignForSaleContainer() {
  ...
  const isVerifyEKYC = searchParams.get('isVerifyEKYC') ?? '';
  const isSignSuccess = searchParams.get('isSignSuccess') ?? '';
  ...
  return (
    <View style={tw.style('flex-1 bg-white h-full')}>
      ...
      <ViewContractESignForSaleScreen
        uri={uri}
        isVerifyEKYC={Boolean(isVerifyEKYC)}
        isSignSuccess={Boolean(isSignSuccess)}
      />
    </View>
  );
}
