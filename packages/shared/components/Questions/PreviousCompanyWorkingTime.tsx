...
const PreviousCompanyWorkingTime = ({stepNumber, control}: QuestionField) => {
  ...
  const {getStep, getPreviousWorkingDuration} = useHandleLoanInformation({
    control,
    stepNumber,
  });
        ...
        <View style={tw.style('flex-1 mt-1')}>
          ...
          {getPreviousWorkingDuration && (
            <Text style={tw.style('text-right mt-2')}>
              {getPreviousWorkingDuration?.years +
                ' years ' +
                getPreviousWorkingDuration?.months +
                ' months'}
              {}
            </Text>
          )}
...
export default PreviousCompanyWorkingTime;
