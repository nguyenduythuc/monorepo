...
export const AutoComplete: React.FC<AutoCompleteProps> = ({ searchValue = '', listResult, title, onChange, onSelect, addressAuto }) => {
    ...
    useEffect(() => { setFilteredResults(listResult) }, [listResult])
    ...
    const getDataList = (query: string) => {
        ...
        return listResult.filter(item => item.name.toLowerCase().includes(query.toLowerCase()));
    };
    ...
    return (
        <>
            <View style={tw.style('mb-2 ')}>
                <TextInputSearch onChangeValue={handleSearchChange} value={searchQuery} />
                {title && <Text style={tw.style('text-lg font-semibold mt-4')}> {title}</Text>}
            </View>
                    ...
                    >
                        <View style={tw.style('flex flex-row py-2')}>
                            ...
                            <View style={tw.style('justify-center items-start pl-1')}>
                                <Text style={tw`text-black ${addressAuto ? 'font-medium' : ''}  text-base pb-0.5`}>
                                    {item.name}
                                </Text>
                                {item.description && <Text style={tw`font-light`}>
                                    {item.description}
                                </Text>}
                            </View>
                        </View>
                        {addressAuto ?? <Icon name="chevron-right" color="black" height={15} />}
                    </TouchableOpacity>
                ))}
            </ScrollView>
        </>
