...
export const AddressModal: React.FC<AddressModalProps> = ({
    ...
    const listAddress: autoCompleteData[] = [
        { _id: 1, name: 'Lotte Center Hà Nội', description: '54 Liễu Giai, Cống Vị, Ba Đình, Hà Nội', code: '1' },
        { _id: 2, name: 'Test 1', description: '54 Liễu Giai, Cống Vị, Ba Đình, Hà Nội', code: '2' },
        { _id: 3, name: 'Test 2', description: '54 Liễu Giai, Cống Vị, Ba Đình, Hà Nội', code: '3' },
        { _id: 4, name: 'Test 3', description: '54 Liễu Giai, Cống Vị, Ba Đình, Hà Nội', code: '4' },
    ];
    ...
    const fillAddress = (type: string, value?: string) => {
        if (type === 'submit') {
            ...
            setAddress(t('AddAddressz.inputAddress'))
        }
                ...
                <View
                    ...
                    <View
                        ...
                        <View style={tw`items-start `}>
                            <TouchableOpacity onPress={() => onCloseAutoAddress()} style={tw`flex-row items-center`}>
                                <Icon name='arrow-left' color='black' disabled></Icon>
                            </TouchableOpacity>
                        </View>
                        <View
                            style={tw`inset-x-20 flex justify-center absolute self-center items-center`}>
                            <Text style={tw`font-semibold text-lg`}>{t('AddAddress.address')}</Text>
                        </View>
                        <View style={tw`flex-1 justify-end items-end`}>
                            <TouchableOpacity onPress={() => setAddressType(!addressAuto)} style={tw`flex-row items-center`}>

                                <Text style={[tw`text-blue-500`]}>{addressAuto ? t('VerifyCustomer.manual') : t('VerifyCustomer.auto')}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                    {addressAuto ?
                        <AddAutoAddress onChange={setAddress} listAutoAddress={listAddress} value={address === t('AddAddress.inputAddress') ? '' : address} changeMode={() => setAddressType(!addressAuto)} autoSubmit={fillAddress} />
                        :
