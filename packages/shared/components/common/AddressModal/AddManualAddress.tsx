...
type ManualAddressModalProps = {
    value?: string;
    manualSubmit: (type: 'reset' | 'submit', value?: string) => void;
};
...
export const AddManualAddress: React.FC<ManualAddressModalProps> = ({
    ...
    const { listCityData, listDistrictData, listWardData, onSelectGetData } = useAddAddress()

    const [detailAddress, setDetailAddress] = useState('')
    const [chooseStep, setChooseStep] = useState({ step: 1, name: t('AddAddress.city'), data: listCityData })
    ...
    const updateLoanStepTitle = (index: number, newTitle: string) => {
            ...
            setChooseStep((prevChooseStep) => {
                switch (chooseStep.step - 1) {
                    ...
                    case 1:
                        ...
                        return { ...prevChooseStep, step: 1, name: t('AddAddress.city'), data: listCityData };
                    case 2:
                        updateLoanStepTitle(2, t('AddAddress.chooseWard'))
                        return { ...prevChooseStep, step: 2, name: t('AddAddress.district'), data: listDistrictData };
                    case 3:
                        updateLoanStepTitle(3, t('AddAddress.noteStreet'))
                        return { ...prevChooseStep, step: 3, name: t('AddAddress.ward'), data: listWardData };
                    default:
        ...
        if (completeAddress?.includes(',')) {
            ...
            switch (type) {
                ...
                case 2:
                    ...
                    setChooseStep({ step: 2, name: t('AddAddress.district'), data: listDistrictData });
                    break;
                case 3:
                    // Add 'Choose Ward' to loanStep and update chooseStep for Ward selection
                    setChooseStep({ step: 3, name: t('AddAddress.ward'), data: listWardData });
                    break;
                case 4:
    ...
    const chooseAddressOption = (id: number | string) => {
        ...
        if (addressDetail) {
            ...
            updateLoanStepTitle(chooseStep.step - 1, addressDetail.name)
    ...
    const onSubmit = () => {
        ...
        const fullAddress = (detailAddress !== '' ? detailAddress + ', ' : detailAddress) + locations.map((location: { title: string }) => location.title) // Extract the title from each object
            .reverse() // Reverse the array to match the desired order
            .join(', ');

        manualSubmit('submit', fullAddress)
    }

    const reverseMergedLocation = (mergedLocation: string) => {
        const cityToWardAddress = mergedLocation.split(',').reverse().splice(0, 3)
        const noteStreetAddress = mergedLocation.split(',').reverse().splice(3, mergedLocation.split(',').reverse().length - 1).reverse().join(',')

        // Split the merged string by commas, trim spaces, and map it back into objects
        let reverseAddress = cityToWardAddress
            .map((location) => ({ title: location.trim() }))
    ...
    return (
        <View style={tw.style('flex-1 mx-4')}>
            <View style={tw`flex-row items-center justify-between `}>
                <Text style={tw`font-semibold text-base`}>{t('AddAddress.selectAddress')}</Text>
                <View style={tw.style('flex-row')}>
                    <CustomButton disabled={chooseStep.step === 1} prefixIcon='undo-icon' variant='text' onPress={onUndo}>{t('AddAddress.undo')}</CustomButton>
                </View>
            ...
            <ScrollView>
                ...
                {chooseStep.step !== 4 ? <ScrollView style={tw.style('flex-1 border-t pt-2 border-gray-200')}>
                    <View style={tw.style('')}>
                        <AutoComplete addressAuto={false} title={chooseStep.name} listResult={chooseStep.data} onSelect={chooseAddressOption}></AutoComplete>
                    </View>

                </ScrollView> : <View style={tw.style(' ml-5')}><TextInputBase value={detailAddress} onChangeText={setDetailAddress} /></View>}

            </ScrollView>
            {chooseStep.step === 4 && <CustomButton onPress={onSubmit} color={'red'} buttonStyle={''}>
