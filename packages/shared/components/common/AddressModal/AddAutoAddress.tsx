...
export const AddAutoAddress: React.FC<AutoAddressModalProps> = ({
    ...
    const selectFromResult = (value: number | string, list: autoCompleteData[]) => {
    ...
    return (<>
        <View style={tw.style('flex-1 mx-4')}>
            <AutoComplete onChange={onChange} addressAuto={true} searchValue={value} listResult={autoAddressList.length !== 0 ? autoAddressList : listAutoAddress} onSelect={selectFromResult} />
        </View>
        <View style={tw.style('flex justify-center items-center rounded-xl pt-3 bg-blue-50 mx-4')}>
            <Icon name='search' size={40}></Icon>
            <Text>{t('AddAddress.notFoundAddress')}</Text>
            <CustomButton iconColor='blue' prefixIcon='add-icon' variant='text' onPress={changeMode}>{t('AddAddress.addAddressManual')}</CustomButton>
        </View>
    </>
    )
}