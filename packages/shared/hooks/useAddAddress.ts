...
const useAddAddress = () => {
  const [getListAutoComplete] = useLazyGetPlaceAutoCompleteQuery();

  const [provinceList] = useGetProvinceListMutation();
  const [districtList] = useGetDistrictListMutation();
  const [wardList] = useGetWardListMutation();

  const [listCityData, setListCityData] = useState<autoCompleteData[] | []>([]);

  const [listDistrictData, setListDistrictData] = useState<
    autoCompleteData[] | []
  >([]);
